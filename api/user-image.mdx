...
<Steps>
  <Step title="Check the profile">
    Call `GET /user/profile`. If `base_image_path` is `null`, the user has no
...
```bash cURL
curl https://api.yourdomain.com/user/profile \
  -H "x-api-key: $SOULDI_API_KEY" \
  -H "Authorization: Bearer $ACCESS_TOKEN"
```

```js fetch
const res = await fetch("https://api.yourdomain.com/user/profile", {
  headers: {
...
<ResponseField name="base_image_path" type="string | null">
  Storage path of the processed base image. `null` means the user hasn't
  uploaded a usable photo yet.
</ResponseField>
...
```json 200 OK
{
  ...
  "base_image_path": "tenants/store_42/users/9b1c.../base.jpg",
...
```bash cURL
curl -X POST https://api.yourdomain.com/user/profile/upload-url \
  -H "x-api-key: $SOULDI_API_KEY" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
...
const res = await fetch(
  "https://api.yourdomain.com/user/profile/upload-url",
  {
    method: "POST",
    headers: {
      "x-api-key": SOULDI_API_KEY,
...
```bash cURL
curl -X PATCH https://api.yourdomain.com/user/profile \
  -H "x-api-key: $SOULDI_API_KEY" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash cURL
# Original base image
curl "https://api.yourdomain.com/user/signed_user_image_url" \
  -H "x-api-key: $SOULDI_API_KEY" \
  -H "Authorization: Bearer $ACCESS_TOKEN"

# Background-removed variant
curl "https://api.yourdomain.com/user/signed_user_image_url?variant=no_bg" \
  -H "x-api-key: $SOULDI_API_KEY" \
  -H "Authorization: Bearer $ACCESS_TOKEN"
```
...
```js fetch
const res = await fetch(
  "https://api.yourdomain.com/user/signed_user_image_url",
  {
    headers: {
      "x-api-key": SOULDI_API_KEY,
      Authorization: `Bearer ${accessToken}`,
